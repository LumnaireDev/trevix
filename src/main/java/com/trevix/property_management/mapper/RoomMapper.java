...
public class RoomMapper {
    ...
    public RoomMapper(PropertyMapper propertyMapper) {
    ...
    public void updateEntity(Room room, RoomUpdateRequest request) {
        ...
        if (request.getWaterFee() != null) {
            room.setWaterFee(request.getWaterFee());
        }
        if (request.getElectricityFee() != null) {
            room.setElectricityFee(request.getElectricityFee());
        }
    ...
    public RoomDetailResponse toDetailResponse(Room room) {
        ...
        if (room.getProperty() != null) {
            response.setProperty(propertyMapper.toResponse(room.getProperty()));
        }
        
        // Calculate occupancy percentage
        if (room.getCapacity() != null && room.getCapacity() > 0) {
            int occupancyPercent = (room.getCurrentOccupancy() * 100) / room.getCapacity();
            response.setOccupancyPercentage(occupancyPercent);
        }
