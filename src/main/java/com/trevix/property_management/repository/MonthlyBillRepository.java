...
public interface MonthlyBillRepository extends JpaRepository<MonthlyBill, UUID> {
    ...
    List<MonthlyBill> findByRoom_Id(UUID roomId);

    List<MonthlyBill> findByStatus(BillStatus status);

    @Query("SELECT b FROM MonthlyBill b WHERE b.property.id = :propertyId ORDER BY b.createdAt DESC")
    List<MonthlyBill> findByPropertyId(@Param("propertyId") UUID propertyId);

    @Query("SELECT b FROM MonthlyBill b WHERE b.property.owner.id = :ownerId ORDER BY b.createdAt DESC")
    List<MonthlyBill> findByOwnerId(@Param("ownerId") UUID ownerId);

    @Query("SELECT b FROM MonthlyBill b WHERE b.tenant.userId = :tenantId AND b.status = 'PENDING'")
    List<MonthlyBill> findPendingByTenant(@Param("tenantId") UUID tenantId);

    @Query("SELECT b FROM MonthlyBill b WHERE b.property.owner.id = :ownerId AND b.status = 'PENDING'")
    List<MonthlyBill> findUnpaidByOwner(@Param("ownerId") UUID ownerId);
    ...
    @Query("SELECT COUNT(b) FROM MonthlyBill b WHERE b.property.id = :propertyId AND b.status = 'PENDING'")
    long countUnpaidByProperty(@Param("propertyId") UUID propertyId);

    @Modifying
    @Transactional
    @Query("UPDATE MonthlyBill b SET b.status = 'OVERDUE' WHERE b.dueDate < :date AND b.status = 'PENDING'")
    void markOverdue(@Param("date") LocalDate date);
}